...
if ($userId) {
    // Completed tasks
$stmtCompleted = $pdo->prepare("SELECT COUNT(*) FROM tasks WHERE status = 'done'");
$stmtCompleted->execute(); // No parameters needed
$completedCount = $stmtCompleted->fetchColumn();

    // Pending tasks
$stmtPending = $pdo->prepare("SELECT COUNT(*) FROM tasks WHERE status = 'pending'");
$stmtPending->execute(); // No parameters needed
$pendingCount = $stmtPending->fetchColumn();

    // In-progress tasks
$stmtInProgress = $pdo->prepare("SELECT COUNT(*) FROM tasks WHERE status = 'in-progress'");
$stmtInProgress->execute(); // No parameters needed
...
<div class="main-content-right">
    <h2>Recent Tasks</h2>
  <br>  <p id="pendingCountMessage"></p> <br>
    <ul id="pendingBookingsList" style="list-style-type: none; padding: 0;">
